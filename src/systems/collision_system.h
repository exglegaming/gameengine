...
// 			std::vector<ecs::entity> entities = get_system_entities();
// 			bool collision_happened;
// 			std::sort(entities.begin(), entities.end(), [&collision_happened](ecs::entity& entity_a, ecs::entity& entity_b) {
// 				auto& collider_a = entity_a.get_component<box_collider_component>();
// 				auto& collider_b = entity_b.get_component<box_collider_component>();

// 				auto& transform_a = entity_a.get_component<transform_component>();
// 				auto& transform_b = entity_b.get_component<transform_component>();

// 				collision_happened = (
// 					transform_a.position.x < transform_b.position.x + collider_b.width &&
// 					transform_a.position.x + collider_a.width > transform_b.position.x &&
// 					transform_a.position.y < transform_b.position.y + collider_b.height &&
// 					transform_a.position.y + collider_b.height > transform_b.position.y
// 				);
